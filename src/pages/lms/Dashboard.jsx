...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        console.log('All enrollments with status and progress:', enrollments.map(e => ({
          id: e?._id,
  ...
  const renderEnrollmentCard = (enrollment) => {
    ...
    const lastAccessed = enrollment.lastAccessed ? new Date(enrollment.lastAccessed).toLocaleDateString() : 'Never';
    const isEnrolled = ['enrolled', 'in_progress', 'completed'].includes(enrollment.status);
      ...
      cover={
        <div style={{ position: 'relative', paddingTop: '56.25%', overflow: 'hidden' }}>
          <img 
            ...
            src={course.thumbnail || 'https://via.placeholder.com/300x169?text=Course+Image'} 
            style={{ 
              position: 'absolute',
              top: 0,
              left: 0,
          ...
          {isEnrolled && (
            <div style={{ 
              ...
              <span><ClockCircleOutlined /> {lastAccessed}</span>
              <span><CheckCircleOutlined /> {completedLessons}/{totalLessons} Lessons</span>
            </div>
          )}
        </div>
      }
    >
      <div style={{ marginBottom: 12 }}>
        <h3 style={{ marginBottom: 4 }}>{course.title || 'Untitled Course'}</h3>
          ...
          <div>
            ...
            <Button 
              ...
              style={{ marginTop: 8 }}
                ...
                className='text-white bg-[#722ed1] hover:bg-[#722ed1]'
            >
  ...
  if (loading && enrollments.length === 0) {
    return (
      <div style={{ padding: '24px' }}>
        <h1 style={{ marginBottom: 24 }}>My Learning Dashboard</h1>
      ...
      <Card style={{ marginBottom: 24 }}>
          ...
          <Col xs={24} sm={12} md={6}>
            <Card>
              <div style={{ textAlign: 'center' }}>
                ...
                <h3>Total Courses</h3>
                <p style={{ fontSize: 24, fontWeight: 'bold' }}>{dashboardStats.totalCourses}</p>
              </div>
            </Card>
          </Col>
          <Col xs={24} sm={12} md={6}>
            <Card>
              <div style={{ textAlign: 'center' }}>
                ...
                <h3>In Progress</h3>
                <p style={{ fontSize: 24, fontWeight: 'bold' }}>{dashboardStats.inProgress}</p>
              </div>
            </Card>
          </Col>
          <Col xs={24} sm={12} md={6}>
            <Card>
              <div style={{ textAlign: 'center' }}>
                ...
                <h3>Completed</h3>
                <p style={{ fontSize: 24, fontWeight: 'bold' }}>{dashboardStats.completed}</p>
              </div>
            </Card>
          </Col>
          <Col xs={24} sm={12} md={6}>
            <Card>
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ fontSize: 24, fontWeight: 'bold' }}>{dashboardStats.totalHours}h</p>
              </div>
            </Card>
          </Col>
        </Row>
        ...
        <div style={{ marginTop: 24 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            ...
            <span>{dashboardStats.currentWeekHours}h / {dashboardStats.weeklyGoal}h</span>
          </div>
          <Progress 
            percent={Math.min(100, (dashboardStats.currentWeekHours / dashboardStats.weeklyGoal) * 100)} 
            showInfo={false}
            strokeColor={dashboardStats.currentWeekHours >= dashboardStats.weeklyGoal ? '#52c41a' : '#1890ff'}
          />
        </div>
      </Card>

      {/* Enrolled Courses */}
      <Card title="My Enrollments" style={{ marginBottom: 24 }}>
        <Row gutter={[16, 16]}>
          {enrollments.length > 0 ? (
            enrollments
              .filter(enrollment => enrollment?.course) // Filter out any enrollments without course data
              .map(enrollment => (
