...
const ReferAndEarn = () => {
  ...
  useEffect(() => {
    ...
    const fetchReferralData = async () => {
      try {
        ...
        // Mock data for demo
        setTimeout(() => {
          setReferralCode('Eklavya' + Math.floor(1000 + Math.random() * 9000));
          setStats({
            totalReferrals: 0,
            activeReferrals: 0,
            earnedRewards: 0
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Join me on Eklavya!',
      text: `Use my referral code ${referralCode} to get started on Eklavya and earn rewards!`,
    ...
    <div className="p-6 max-w-7xl mx-auto min-h-screen">
      ...
      >
        <Space direction="vertical" size="small" className="w-full dark:text-gray-300">
          ...
          <div>• Eklavya reserves the right to modify or terminate the program at any time.</div>
        </Space>
      </Card>
    </div>
  );
