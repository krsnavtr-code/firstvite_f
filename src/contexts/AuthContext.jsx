...
export function AuthProvider({ children }) {
      ...
      try {
        ...
        try {
          ...
          if (error.response?.status === 401) {
            console.log('Authentication token is invalid or expired, attempting refresh...');
  ...
  const login = async (email, password) => {
    try {
      ...
      if (response.data && response.data.success && response.data.token && response.data.refreshToken) {
